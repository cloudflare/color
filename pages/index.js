...
const getColors = () => {
  const randomColor = this.state.palette? this.state.palette[randomPaletteColor]: randomHex()
  let randomBg = this.state.palette? this.state.palette[randomPaletteColor1]: randomHex()
  while(chroma.contrast(randomColor, randomBg) < 4.5 ) {
    randomBg = this.state.palette[Math.round(Math.random() * (this.state.palette.length - 1))]
  }
  return [randomColor, randomBg]
}

  const randomPaletteColor = Math.round(Math.random() * (this.state.palette.length - 1))
  const randomPaletteColor1 = Math.round(Math.random() * (this.state.palette.length - 1))
  const randomPaletteColor2 = Math.round(Math.random() * (this.state.palette.length - 1))
  const randomPaletteColor3 = Math.round(Math.random() * (this.state.palette.length - 1))
  const randomPaletteColor4 = Math.round(Math.random() * (this.state.palette.length - 1))
  const randomPaletteColor5 = Math.round(Math.random() * (this.state.palette.length - 1))
  ...
  const randomBorderColor = this.state.palette? this.state.palette[randomPaletteColor2]: randomHex()
  ...
  if (this.state.palette) {
    ...
    randomTextShadow = randomBool? this.state.palette[randomPaletteColor3]: 'transparent'
    randomBoxShadow = randomBool? this.state.palette[randomPaletteColor4]: 'transparent'
  }
    ...
    return (
      ...
      <Flex>
        ...
        <Div maxWidth='40em' mx='auto'>
          ...
          <Div mt={5} p={4} bg={data.bg} display='grid' style={{ gridTemplateColumns: 'repeat(12, 1fr)', rowGap: '2em' }}>
